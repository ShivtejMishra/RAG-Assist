...
import {
  ...
  Globe, Cpu, HardDrive, Search, BarChart2, Layers,
...
const DashboardPreview: React.FC = () => (
  <div className="w-full rounded-2xl overflow-hidden border border-slate-200/60 dark:border-white/10 shadow-2xl shadow-black/20 dark:shadow-black/50 bg-white dark:bg-slate-900 select-none pointer-events-none">
    {/* Fake window chrome */}
    <div className="flex items-center gap-1.5 px-4 py-3 bg-slate-100 dark:bg-slate-800/80 border-b border-slate-200 dark:border-slate-700/50">
      <span className="w-3 h-3 rounded-full bg-red-400" />
      <span className="w-3 h-3 rounded-full bg-amber-400" />
      <span className="w-3 h-3 rounded-full bg-emerald-400" />
      <span className="ml-3 text-[10px] text-slate-500 dark:text-slate-400 font-mono">RAGAssist — Analytics Dashboard</span>
    </div>

    {/* Fake dashboard content */}
    <div className="flex h-52 sm:h-64">
      {/* Fake sidebar */}
      <div className="w-14 sm:w-20 bg-slate-50 dark:bg-slate-900/80 border-r border-slate-200 dark:border-slate-800 flex flex-col items-center pt-4 gap-3">
        {[Database, BarChart2, MessageSquare, FileText].map((Icon, i) => (
          <div key={i} className={`w-7 h-7 rounded-lg flex items-center justify-center ${i === 0 ? 'bg-brand-600 text-white' : 'bg-slate-200 dark:bg-slate-800 text-slate-400 dark:text-slate-500'}`}>
      ...
      {/* Fake main area */}
      <div className="flex-1 p-3 sm:p-4 bg-slate-50 dark:bg-slate-950 overflow-hidden">
        {/* Fake stat cards */}
        <div className="grid grid-cols-4 gap-2 mb-3">
          {[
            { label: 'Documents', val: '24', color: 'bg-brand-500/10 text-brand-500' },
            { label: 'Chunks', val: '1.2K', color: 'bg-indigo-500/10 text-indigo-500' },
            { label: 'Chats', val: '18', color: 'bg-purple-500/10 text-purple-500' },
            { label: 'Size', val: '48 MB', color: 'bg-cyan-500/10 text-cyan-500' },
          ].map((s) => (
            <div key={s.label} className="bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-xl p-2 flex flex-col gap-0.5">
        ...
        <div className="grid grid-cols-3 gap-2">
          {/* Activity log */}
          <div className="col-span-2 bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-xl p-2.5">
            <div className="text-[8px] font-bold uppercase text-slate-400 mb-2">Activity Log</div>
            {['Vectorization Complete', 'Document Processed', 'Chat Session Active'].map((t, i) => (
              <div key={i} className="flex items-center gap-1.5 mb-1.5">
                <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 flex-shrink-0" />
                <span className="text-[9px] text-slate-600 dark:text-slate-400 truncate">{t}</span>
              </div>
            ))}
          </div>
          {/* System status */}
          <div className="bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-xl p-2.5">
            <div className="text-[8px] font-bold uppercase text-slate-400 mb-2">Status</div>
            {['Gemini LLM', 'Qdrant', 'MongoDB'].map((s) => (
              <div key={s} className="flex items-center justify-between mb-1.5">
                ...
                <span className="text-[8px] text-emerald-500 font-bold">ON</span>
              </div>
            ))}
          </div>
        </div>
